...
exports.EscalaGeneratorService = void 0;
const core_1 = require("@angular/core");
let EscalaGeneratorService = (() => {
    ...
    let _classThis;
    var EscalaGeneratorService = class {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            EscalaGeneratorService = _classThis = _classDescriptor.value;
         ...
         */
        gerarEscalaMensalCached(funcionarios, ano, mes, opcoes) {
            const configStr = [
         ...
         */
        gerarEscalaMensal(funcionarios, ano, mes, opcoes) {
            ...
            const feriadosFechados = new Set();
            const feriadosAbertos = new Set();
            ...
            const feriadoTrabalhadoresMap = new Map();
            ...
            feriadosAbertos.forEach((fDia) => {
                ...
                const escaladosFeriado = new Set();
            ...
            const funcIndexNoTurno = new Map();
            ...
            funcionarios.forEach((func, idx) => {
                ...
                const domingosFolgaSet = new Set();
                ...
                const diasFolgaUteisGarantidas = new Set();
                ...
                const afastamentosFuncMap = new Map();
            ...
            if (eFiscalDeCaixa) {
                domingos.forEach(dDom => {
                    if (!feriadosFechados.has(dDom) && itens.length >= 2) {
                        ...
                        if (trabDom < 2) {
                            for (const item of itens) {
                                if (itens.filter(i => i.dias[dDom] === 'TD' || i.dias[dDom] === 'TF' || i.dias[dDom] === 'T').length >= 2)
                                    break;
         ...
         */
        _agruparPorCategoriaTurno(funcionarios) {
         ...
         */
        _getTipoTrabalho(dia, ano, mes, feriadosAbertos) {
         ...
         */
        _ajustarCoberturaPadaria(itens, totalDias, ano, mes, feriadosAbertos, feriadosFechados) {
            if (itens.length <= 1)
                return;
            const maxFolgasDia = Math.max(1, Math.ceil(itens.length / 6));
            for (let dia = 1; dia <= totalDias; dia++) {
                const isDomingo = (new Date(ano, mes - 1, dia).getDay() === 0);
                if (isDomingo || feriadosFechados.has(dia))
                    continue;
                ...
                if (folgando.length > maxFolgasDia) {
                    for (let k = 0; k < folgando.length && itens.filter(i => i.dias[dia] === 'F' || i.dias[dia] === 'FE').length > maxFolgasDia; k++) {
                        ...
                        for (const off of offsetsPrioridade) {
                            ...
                            if (candidate >= 1 && candidate <= totalDias)
                                dAlts.push(candidate);
                        }
                                    ...
                                    if (maxConsec <= 6) {
                                        ...
                                        for (const otherItem of itens) {
                                            if (otherItem.matricula !== item.matricula && (otherItem.dias[dia] === 'T' || otherItem.dias[dia] === 'TF') && otherItem.dias[dAlt] === 'F') {
                                                ...
                                                for (let d = 1; d <= totalDias; d++) {
                                                    if (item.dias[d] === 'T' || item.dias[d] === 'TD' || item.dias[d] === 'TF') {
                                                        c1++;
                                                        if (c1 > mc1)
                                                            mc1 = c1;
                                                    }
                                                        ...
                                                        c1 = 0;
                                                    if (otherItem.dias[d] === 'T' || otherItem.dias[d] === 'TD' || otherItem.dias[d] === 'TF') {
                                                        c2++;
                                                        if (c2 > mc2)
                                                            mc2 = c2;
                                                    }
                                                ...
                                                if (mc1 <= 6 && mc2 <= 6) {
                                                    ...
                                                    item.dias[dia] = 'F';
                                                    item.dias[dAlt] = this._getTipoTrabalho(dAlt, ano, mes, feriadosAbertos);
                                                    otherItem.dias[dia] = this._getTipoTrabalho(dia, ano, mes, feriadosAbertos);
                                                    otherItem.dias[dAlt] = 'F';
                                                }
                                            }
                                        }
                                        if (diaSemFolga !== -1)
         ...
         */
        _ajustarLimiteFolgasMensais(itens, totalDias, ano, mes, feriadosAbertos) {
            const domingosNoMes = [];
            for (let d = 1; d <= totalDias; d++) {
                if (new Date(ano, mes - 1, d).getDay() === 0)
                    domingosNoMes.push(d);
            }
         ...
         */
        _sanitizarTravaCLT6Dias(itens, totalDias, ano, mes, feriadosAbertos) {
            itens.forEach(item => {
                let consecutivos = 0;
                for (let dia = 1; dia <= totalDias; dia++) {
         ...
         */
        _ajustarCoberturaMinima(itens, totalDias, minPorDia, ano, mes, feriadosAbertos, minFeriado) {
            if (itens.length <= 1)
                return;
            for (let dia = 1; dia <= totalDias; dia++) {
                        ...
                        if (maxConsec > 6) {
                            ...
                            if (!compensou) {
                                ...
                                for (let d = 1; d <= totalDias; d++) {
                                    ...
                                    if (s === 'T' || s === 'TD' || s === 'TF') {
                                        curConsecAfter++;
                                        if (curConsecAfter > maxConsecAfter)
                                            maxConsecAfter = curConsecAfter;
                                    }
                                    else {
                                        curConsecAfter = 0;
                                    }
         ...
         */
        validarEscala(itens, ano, mes, minRequerido = 2, turnosConfigs = [], feriados = [], historicoMesAnterior) {
            const totalDias = new Date(ano, mes, 0).getDate();
            const erros = [];
            const coberturaPorDia = {};
            const feriadosFechadosVal = new Set();
            const feriadosAbertos = new Set();
            ...
            itens.forEach(item => {
                ...
                const domingosNoMesVal = [];
                for (let d = 1; d <= totalDias; d++) {
                    if (new Date(ano, mes - 1, d).getDay() === 0)
                        domingosNoMesVal.push(d);
                }
            ...
            const totalErros = erros.filter(e => e.tipo === 'ERRO_COBERTURA' ||
                ...
                e.tipo === 'ERRO_CARGA_HORARIA_MENSAL').length;
         ...
         */
        calcularCargaHorariaLiquida(entrada, saida, intervaloMinutos) {
         ...
         */
        calcularPresencaPorFaixaHoraria(itens, turnosConfigs, dia) {
            const horasFaixas = [
                '07:00', '08:00', '09:00', '10:00', '11:00', '12:00',
                '13:00', '14:00', '15:00', '16:00', '17:00', '18:00',
                '19:00', '20:00', '21:00', '22:00'
            ];
            const resultado = horasFaixas.map(hStr => ({
                horaStr: hStr,
         ...
         */
        calcularResumoMetrics(itens, funcionarios, turnosConfigs, ano, mes) {
            const totalDias = new Date(ano, mes, 0).getDate();
        ...
        _getMinutosEntradaSaida(item, turnosConfigs) {
            let entradaMin = 8 * 60;
         ...
         */
        extrairHistoricoMesAnterior(itens) {
            const res = {};
            if (!itens)
                return res;
            itens.forEach(item => {
    ...
    return EscalaGeneratorService = _classThis;
})();
exports.EscalaGeneratorService = EscalaGeneratorService;
