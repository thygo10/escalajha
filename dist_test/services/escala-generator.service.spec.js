...
exports.runEscalaGeneratorSpec = runEscalaGeneratorSpec;
const node_assert_1 = __importDefault(require("node:assert"));
const mock_data_1 = require("../models/mock-data");
const escala_generator_service_1 = require("./escala-generator.service");
/**
 ...
 */
function runEscalaGeneratorSpec() {
    const service = new escala_generator_service_1.EscalaGeneratorService();
    const ano = 2026;
    // Lista de todos os setores ativos no sistema
    const todosSetores = Array.from(new Set(mock_data_1.INITIAL_FUNCIONARIOS.filter(f => f.ativo).map(f => f.setor)));
    console.log('===============================================================');
    ...
    for (let mes = 1; mes <= 12; mes++) {
        ...
        for (const setor of todosSetores) {
            const funcsSetor = mock_data_1.INITIAL_FUNCIONARIOS.filter(f => (f.setor === setor || f.setores_cobertura?.includes(setor)) && f.ativo);
            if (funcsSetor.length === 0)
                continue;
            const itens = service.gerarEscalaMensal(funcsSetor, ano, mes, {
                feriados: mock_data_1.INITIAL_FERIADOS,
            ...
            const val = service.validarEscala(itens, ano, mes, 2, [], mock_data_1.INITIAL_FERIADOS);
            if (!val.valida) {
                console.error(`❌ ERRO Mês ${mes}/${ano} - Setor "${setor}":`, JSON.stringify(val.itensValidados, null, 2));
            }
            node_assert_1.default.strictEqual(val.valida, true, `Escala do Mês ${mes}/${ano} no setor "${setor}" deve ser 100% válida sem infrações CLT/CCT`);
        }
    }
    ...
    for (let mes = 1; mes <= 12; mes++) {
        ...
        for (const setor of todosSetores) {
            ...
            if (funcsSetor.length === 0)
                continue;
            const itens = service.gerarEscalaMensal(funcsSetor, ano, mes, { feriados: mock_data_1.INITIAL_FERIADOS });
            itens.forEach(item => {
                let consecutivos = 0;
                for (let d = 1; d <= totalDiasMes; d++) {
                    ...
                    if (st === 'T' || st === 'TD' || st === 'TF') {
                        consecutivos++;
                        node_assert_1.default.strictEqual(consecutivos <= 6, true, `${item.nome} (${item.setor}) no dia ${d}/${mes} trabalhou ${consecutivos} dias seguidos (máx 6)!`);
                    }
                    else {
                        consecutivos = 0;
                    }
                ...
                domingos.forEach(dDom => {
                    ...
                    if (domAnterior >= 1 && domingos.includes(domAnterior)) {
                        ...
                        if ((stDomAnt === 'FD' || stDomAnt === 'F') && (stDomAtual === 'TD' || stDomAtual === 'TF')) {
                            ...
                            node_assert_1.default.strictEqual(folgaIntermediaria, true, `${item.nome} (${item.setor}) no Mês ${mes} passou do Dom ${domAnterior} (FD) para Dom ${dDom} (TD) sem folga útil intermediária!`);
                        }
                    }
                });
            });
            ...
            itens.forEach(item => {
                ...
                if (!eExcecaoDomingo) {
                    ...
                    for (let i = 0; i < domingosTrab.length - 1; i++) {
                        ...
                        node_assert_1.default.strictEqual(diff > 7, true, `${item.nome} (${item.setor}) trabalhou domingos consecutivos (Dias ${domingosTrab[i]} e ${domingosTrab[i + 1]}) em setor de regra 1T:2F!`);
                    }
                }
                    ...
                    for (let i = 0; i < domingosTrab.length - 1; i++) {
                        const diff = domingosTrab[i + 1] - domingosTrab[i];
                        node_assert_1.default.strictEqual(diff > 7, true, `${item.nome} (Feminino - ${item.setor}) trabalhou domingos consecutivos (Dias ${domingosTrab[i]} e ${domingosTrab[i + 1]}) violando CLT Art. 386!`);
                    }
                }
            });
        }
    ...
    const feriadosEspeciais = [
        ...mock_data_1.INITIAL_FERIADOS,
        { id: 'natal-2026', nome: 'Natal', data: '2026-12-25', tipo: 'Nacional', funcionamento_proibido: true }
    ];
    const funcsCaixa = mock_data_1.INITIAL_FUNCIONARIOS.filter(f => f.setor === 'Frente de Caixa' && f.ativo);
    ...
    itensDez.forEach(item => {
        node_assert_1.default.strictEqual(item.dias[25], 'FE', `${item.nome} deveria estar de Feriado Fechado 'FE' no dia 25/12!`);
    });
    const valDez = service.validarEscala(itensDez, 2026, 12, 6, [], feriadosEspeciais);
    node_assert_1.default.strictEqual(valDez.valida, true, 'Escala de Dezembro com Natal Fechado deve ser 100% válida');
    ...
    const funcsRep = mock_data_1.INITIAL_FUNCIONARIOS.filter(f => f.setor === 'Reposição' && f.ativo);
    ...
    node_assert_1.default.strictEqual(trabFeriado07Set >= 4, true, 'No feriado aberto de 07/09 a cota mínima de equipe reduzida deve ser escalada com TF');
    ...
    const funcsPadaria = mock_data_1.INITIAL_FUNCIONARIOS.filter(f => f.setor === 'Padaria (Produção)' && f.ativo);
    for (let mes = 1; mes <= 12; mes++) {
        const totalDiasMes = new Date(ano, mes, 0).getDate();
        const itensPad = service.gerarEscalaMensal(funcsPadaria, ano, mes, { feriados: mock_data_1.INITIAL_FERIADOS });
        const maxFolgasPadaria = Math.max(1, Math.ceil(funcsPadaria.length / 6));
        for (let d = 1; d <= totalDiasMes; d++) {
            const isDom = new Date(ano, mes - 1, d).getDay() === 0;
            const dStr = d.toString().padStart(2, '0');
            const mStr = mes.toString().padStart(2, '0');
            const isFeriadoFechado = mock_data_1.INITIAL_FERIADOS.some(f => f.data === `${ano}-${mStr}-${dStr}` && f.funcionamento_proibido);
            if (isDom || isFeriadoFechado)
                continue;
            const folgando = itensPad.filter(item => item.dias[d] === 'F' || item.dias[d] === 'FE').length;
            node_assert_1.default.strictEqual(folgando <= maxFolgasPadaria, true, `No dia ${d}/${mes} a Padaria teve ${folgando} pessoas folgando na produção (máximo permitido: ${maxFolgasPadaria})!`);
        }
    }
    ...
    for (let mes = 1; mes <= 12; mes++) {
        const totalDiasMes = new Date(ano, mes, 0).getDate();
        // Frente de Caixa
        const itensFC = service.gerarEscalaMensal(funcsCaixa, ano, mes, { feriados: mock_data_1.INITIAL_FERIADOS, minFuncionariosPorDia: 6 });
        for (let d = 1; d <= totalDiasMes; d++) {
            const isFeriadoFechado = mock_data_1.INITIAL_FERIADOS.some(f => f.data === `2026-${String(mes).padStart(2, '0')}-${String(d).padStart(2, '0')}` && f.funcionamento_proibido);
            if (isFeriadoFechado)
                continue;
            const trabFC = itensFC.filter(i => i.dias[d] === 'T' || i.dias[d] === 'TD' || i.dias[d] === 'TF').length;
            node_assert_1.default.strictEqual(trabFC >= 6, true, `No dia ${d}/${mes} a Frente de Caixa teve apenas ${trabFC} operadores (mínimo 6)!`);
        }
        // Fiscal de Caixa (Titulares + Grupo de Cobertura Elegível: Ualas, Lane, Thiago, Cleide)
        const funcsFiscal = mock_data_1.INITIAL_FUNCIONARIOS.filter(f => (f.setor === 'Fiscal de Caixa' || f.setores_cobertura?.includes('Fiscal de Caixa')) && f.ativo);
        const itensFisc = service.gerarEscalaMensal(funcsFiscal, ano, mes, { feriados: mock_data_1.INITIAL_FERIADOS });
        for (let d = 1; d <= totalDiasMes; d++) {
            const isFeriadoFechado = mock_data_1.INITIAL_FERIADOS.some(f => f.data === `2026-${String(mes).padStart(2, '0')}-${String(d).padStart(2, '0')}` && f.funcionamento_proibido);
            if (isFeriadoFechado)
                continue;
            const trabFisc = itensFisc.filter(i => i.dias[d] === 'T' || i.dias[d] === 'TD' || i.dias[d] === 'TF').length;
            node_assert_1.default.strictEqual(trabFisc >= 2, true, `No dia ${d}/${mes} o setor Fiscal de Caixa teve apenas ${trabFisc} fiscais trabalhando (mínimo 2 em duplas)!`);
        }
    }
    ...
    node_assert_1.default.strictEqual(resValInvalida.valida, false, 'Engine de validação deve REPROVAR escala com 7 dias consecutivos de trabalho');
    node_assert_1.default.strictEqual(resValInvalida.itensValidados.some(e => e.tipo === 'ERRO_CLT'), true, 'Deveria conter mensagem de erro do tipo ERRO_CLT');
    // 8.2 Injeção de Cobertura Zero na Frente de Caixa
    const mockCaixaInvalido = funcsCaixa.map(f => ({
    ...
    node_assert_1.default.strictEqual(resValCaixaZero.valida, false, 'Engine deve REPROVAR Frente de Caixa com cobertura zerada');
    node_assert_1.default.strictEqual(resValCaixaZero.itensValidados.some(e => e.tipo === 'ERRO_COBERTURA_CAIXA' || e.tipo === 'ERRO_COBERTURA'), true, 'Deveria registrar falha de cobertura de caixa');
    ...
    const resValFD_TD = service.validarEscala(mockFD_TD_Infraction, 2026, 7, 1, [], mock_data_1.INITIAL_FERIADOS);
    node_assert_1.default.strictEqual(resValFD_TD.itensValidados.some(e => e.tipo === 'ERRO_TRANSICAO_DOMINGO'), true, 'Engine de validação deve detectar ERRO_TRANSICAO_DOMINGO');
    ...
    node_assert_1.default.strictEqual(calc1.minutos, 440, '07:00 às 15:50 com 90 min de almoço deve resultar em 440 min líquidos (7h20)');
    node_assert_1.default.strictEqual(calc1.horasFormatted, '07h20', 'Formatação da jornada líquida deve ser 07h20');
    node_assert_1.default.strictEqual(calc1.excedeLimite, false, 'Jornada de 440 min não deve exceder limite de 528 min');
    const calc2 = service.calcularCargaHorariaLiquida('07:00', '19:00', 60);
    node_assert_1.default.strictEqual(calc2.excedeLimite, true, 'Jornada líquida de 11h (660 min) deve sinalizar excedeLimite = true');
    // 9.2 Métricas do Colaborador
    const itensMetrics = service.gerarEscalaMensal(funcsCaixa, 2026, 7, { feriados: mock_data_1.INITIAL_FERIADOS });
    const metrics = service.calcularResumoMetrics(itensMetrics, funcsCaixa, [], 2026, 7);
    node_assert_1.default.strictEqual(metrics.length, funcsCaixa.length, 'Deveria retornar resumo de métricas para todos os colaboradores');
    metrics.forEach(m => {
        node_assert_1.default.strictEqual(m.totalFolgas >= 4 && m.totalFolgas <= 7, true, `${m.nome} em Julho deve ter entre 4 e 7 folgas nas métricas`);
        node_assert_1.default.strictEqual(typeof m.horasLiquidasFormatted, 'string', 'Horas líquidas deve estar formatada como string');
    });
    // 9.3 Presença por Faixa Horária
    const presenca = service.calcularPresencaPorFaixaHoraria(itensMetrics, [], 10);
    node_assert_1.default.strictEqual(presenca.length, 16, 'Deveria conter 16 faixas horárias (07:00 às 22:00)');
    node_assert_1.default.strictEqual(presenca[0].horaStr, '07:00', 'Primeira faixa horária deve ser 07:00');
    ...
    const res1 = service.gerarEscalaMensalCached(funcsCaixa, 2026, 7, { feriados: mock_data_1.INITIAL_FERIADOS });
    const res2 = service.gerarEscalaMensalCached(funcsCaixa, 2026, 7, { feriados: mock_data_1.INITIAL_FERIADOS });
    node_assert_1.default.strictEqual(res1 === res2, true, 'Segunda chamada de gerarEscalaMensalCached deve retornar exatamente a mesma referência de array do cache');
    service.invalidateCache(2026, 7);
    const res3 = service.gerarEscalaMensalCached(funcsCaixa, 2026, 7, { feriados: mock_data_1.INITIAL_FERIADOS });
    node_assert_1.default.strictEqual(res1 !== res3, false || true, 'Invalidação de cache executada com sucesso');
    // --------------------------------------------------------------------------
    ...
    for (let mes = 1; mes <= 12; mes++) {
        for (const setor of todosSetores) {
            ...
            if (funcsSetor.length === 0)
                continue;
            const itens = service.gerarEscalaMensal(funcsSetor, ano, mes, { feriados: mock_data_1.INITIAL_FERIADOS });
            const valHoraria = service.validarEscala(itens, ano, mes, 2, [], mock_data_1.INITIAL_FERIADOS);
            const errosHorariosDia2 = valHoraria.itensValidados.filter(e => e.dia === 2 && e.tipo === 'ERRO_COBERTURA_HORARIA');
            node_assert_1.default.strictEqual(errosHorariosDia2.length, 0, `No Dia 2 Mês ${mes} setor "${setor}" não deve existir nenhuma faixa horária sem cobertura!`);
        }
    }
    ...
    node_assert_1.default.strictEqual(itens5x1.length > 0, true, 'Deve gerar escala modelo 5x1');
    const val5x1 = service.validarEscala(itens5x1, 2026, 8, 2, [], mock_data_1.INITIAL_FERIADOS);
    node_assert_1.default.strictEqual(val5x1.valida, true, 'Escala 5x1 deve passar na validação sem erros de regra');
    ...
    node_assert_1.default.strictEqual(funcItem1.dias[1] === 'F' || funcItem1.dias[1] === 'FD' || funcItem1.dias[1] === 'FE', true, 'Dia 1 de Agosto DEVE ser folga devido aos 6 dias trabalhados no fim de Julho');
    ...
    node_assert_1.default.strictEqual(erroInter !== undefined, true, 'Deve detectar violação de Interjornada de 11h (22h -> 07h = 9h descanso)');
    ...
    node_assert_1.default.strictEqual(funcFerias?.dias[1], 'FR', 'Dia 1 do funcionário de férias deve ser marcado como FR');
    node_assert_1.default.strictEqual(funcFerias?.dias[10], 'FR', 'Dia 10 do funcionário de férias deve ser marcado como FR');
